...
const consoleWarn = (string: string): void => console.log(compose(boldConsoleText, redConsoleText)(string));
const readFile = (fileName: string): string =>
    fs.readFileSync(path.resolve(__dirname + `/boilerplate/${fileName}.boilerplate`), 'utf8');
...
const fillTemplates = (componentString: string): { component: string; stories: string; test: string } => {
...
program
    ...
    .action(function(componentName: string, cmd: { subDirectory: string }): void {
        ...
        const filePathBuilder = (fileExtension?: string): string =>
            `${COMPONENTS_DIR_PATH}${
                cmd.subDirectory ? `/${cmd.subDirectory.charAt(0) + capitalize(cmd.subDirectory.slice(1))}` : ''
            }/${component}/${component}${fileExtension ? `.${fileExtension}` : ''}.tsx`;

        if (cmd.subDirectory) {
            if (cmd.subDirectory.charAt(0) !== '/')
                return consoleWarn('The value passed to the --sub-directory option should start with a "/".');

            const parentDirPath = pathBuilder(`/${cmd.subDirectory.charAt(0) + capitalize(cmd.subDirectory.slice(1))}`);
            componentDirPath = parentDirPath + componentFolderSubPath;

            executeUnlessFileExists(parentDirPath, (): void => {
                executeCmd(`mkdir ${parentDirPath}`);
            });
        }

        executeUnlessFileExists(componentDirPath, (): void => {
            executeCmd(`mkdir ${componentDirPath}`);
        });

        if (fs.existsSync(filePathBuilder())) return consoleWarn('Component already exists.');
